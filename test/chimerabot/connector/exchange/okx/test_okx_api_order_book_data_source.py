...
import chimerabot.connector.exchange.okx.okx_constants as CONSTANTS
import chimerabot.connector.exchange.okx.okx_web_utils as web_utils
from chimerabot.connector.exchange.okx.okx_api_order_book_data_source import OkxAPIOrderBookDataSource
from chimerabot.connector.exchange.okx.okx_exchange import OkxExchange
...
class OkxAPIOrderBookDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    @aioresponses()
    @patch("chimerabot.connector.exchange.okx.okx_api_order_book_data_source"
    ...
    async def test_subscribe_to_trading_pair_successful(self):
        ...
        self.connector._set_trading_pair_symbol_map(
            bidict({f"{self.base_asset}-{self.quote_asset}": self.trading_pair, "ETH-USDT": new_pair})
        )
        ...
        # OKX sends 2 messages: trades and books
    ...
    async def test_unsubscribe_from_trading_pair_successful(self):
        ...
        self.assertTrue(result)
        # OKX sends 1 message for unsubscribe (both channels in one request)
