...
import chimerabot.connector.exchange.derive.derive_constants as CONSTANTS
import chimerabot.connector.exchange.derive.derive_web_utils as web_utils
...
from chimerabot.core.data_type.in_flight_order import InFlightOrder
...
class DeriveExchangeTests(AbstractExchangeConnectorTests.ExchangeConnectorTests):
    ...
    async def _run_initialize_rate_limits_with_mocked_throttler(self, account_type, expected_limit):
        ...
        with patch("chimerabot.connector.exchange.derive.derive_exchange.deepcopy", return_value=[]):
    ...
    async def test_start_network_starts_rate_limits_polling_loop(self):
        with patch("chimerabot.connector.exchange.derive.derive_exchange.safe_ensure_future") as mock_safe_ensure_future:
