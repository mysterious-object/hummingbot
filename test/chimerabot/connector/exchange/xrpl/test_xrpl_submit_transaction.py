...
class TestXRPLSubmitTransaction(IsolatedAsyncioTestCase):
    ...
    async def test_submit_transaction_success(self, mock_client_class):
        ...
        with patch("chimerabot.connector.exchange.xrpl.xrpl_exchange.sign", return_value=mock_signed_tx) as mock_sign:
            with patch(
                "chimerabot.connector.exchange.xrpl.xrpl_exchange.async_submit_and_wait", return_value=mock_response
    ...
    @patch("chimerabot.connector.exchange.xrpl.xrpl_exchange.AsyncWebsocketClient")
    @patch("chimerabot.connector.exchange.xrpl.xrpl_constants.PLACE_ORDER_MAX_RETRY", 1)
    async def test_submit_transaction_error_response(self, mock_client_class):
        ...
        # Patch sign and submit_and_wait methods
        with patch("chimerabot.connector.exchange.xrpl.xrpl_exchange.sign", return_value=mock_signed_tx):
            with patch(
                "chimerabot.connector.exchange.xrpl.xrpl_exchange.async_submit_and_wait", return_value=error_response
            ):
    ...
    async def test_submit_transaction_exception(self, mock_client_class):
        ...
        with patch("chimerabot.connector.exchange.xrpl.xrpl_exchange.sign", return_value=mock_signed_tx):
            with patch(
                "chimerabot.connector.exchange.xrpl.xrpl_exchange.async_submit_and_wait",
