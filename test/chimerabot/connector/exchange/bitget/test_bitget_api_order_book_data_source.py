...
import chimerabot.connector.exchange.bitget.bitget_web_utils as web_utils
...
from chimerabot.connector.exchange.bitget.bitget_exchange import BitgetExchange
...
class BitgetAPIOrderBookDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_subscribe_to_trading_pair_successful(self):
        ...
        ex_new_pair = "ETHUSDT"
        self.connector._set_trading_pair_symbol_map(
            bidict({self.exchange_trading_pair: self.trading_pair, ex_new_pair: new_pair})
        )
        ...
        self.assertEqual(1, mock_ws.send.call_count)  # 1 message with batched topics
        self.assertTrue(
