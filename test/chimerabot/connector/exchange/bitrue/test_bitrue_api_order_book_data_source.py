...
class BitrueAPIOrderBookDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    @aioresponses()
    @patch(
        "chimerabot.connector.exchange.bitrue.bitrue_api_order_book_data_source" ".BitrueAPIOrderBookDataSource._sleep"
    )
    ...
    async def test_unsubscribe_from_trading_pair_successful(self):
        ...
        self.assertEqual(1, mock_ws.send.call_count)  # 1 channel: orderbook
        self.assertTrue(
            self._is_logged("INFO", f"Unsubscribed from public order book channel of {self.trading_pair}...")
        )
