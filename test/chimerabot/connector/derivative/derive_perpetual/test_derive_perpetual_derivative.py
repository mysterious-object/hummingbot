...
from chimerabot.connector.test_support.perpetual_derivative_test import AbstractPerpetualDerivativeTests
...
from chimerabot.core.data_type.in_flight_order import InFlightOrder, OrderState
from chimerabot.core.data_type.trade_fee import DeductedFromReturnsTradeFee, TokenAmount, TradeFeeBase
from chimerabot.core.event.event_logger import EventLogger
...
class DerivePerpetualDerivativeTests(AbstractPerpetualDerivativeTests.PerpetualDerivativeTests):
    ...
    async def _run_initialize_rate_limits_with_mocked_throttler(self, account_type, expected_limit):
        ...
        with patch("chimerabot.connector.derivative.derive_perpetual.derive_perpetual_derivative.deepcopy", return_value=[]):
    ...
    async def test_start_network_starts_rate_limits_polling_loop(self):
        with patch("chimerabot.connector.derivative.derive_perpetual.derive_perpetual_derivative.safe_ensure_future") as mock_safe_ensure_future:
    ...
    @patch(
        "chimerabot.connector.derivative.derive_perpetual.derive_perpetual_api_order_book_data_source.DerivePerpetualAPIOrderBookDataSource._next_funding_time")
    ...
    @patch("chimerabot.connector.derivative.derive_perpetual.derive_perpetual_derivative.DerivePerpetualDerivative._update_positions")
    @aioresponses()
    ...
    @aioresponses()
    @patch("chimerabot.connector.time_synchronizer.TimeSynchronizer._current_seconds_counter")
