...
import chimerabot.connector.derivative.derive_perpetual.derive_perpetual_constants as CONSTANTS
import chimerabot.connector.derivative.derive_perpetual.derive_perpetual_web_utils as web_utils
...
from chimerabot.connector.derivative.derive_perpetual.derive_perpetual_api_order_book_data_source import (
    DerivePerpetualAPIOrderBookDataSource,
)
from chimerabot.connector.derivative.derive_perpetual.derive_perpetual_derivative import DerivePerpetualDerivative
from chimerabot.connector.test_support.network_mocking_assistant import NetworkMockingAssistant
from chimerabot.connector.trading_rule import TradingRule
from chimerabot.core.data_type.funding_info import FundingInfo, FundingInfoUpdate
from chimerabot.core.data_type.order_book import OrderBook
from chimerabot.core.data_type.order_book_message import OrderBookMessage, OrderBookMessageType
...
class DeriveAPIOrderBookDataSourceTests(IsolatedAsyncioWrapperTestCase):
    ...
    @patch("chimerabot.connector.derivative.derive_perpetual.derive_perpetual_api_order_book_data_source"
           ".DerivePerpetualAPIOrderBookDataSource._request_order_book_snapshot", new_callable=AsyncMock)
    ...
    @patch("chimerabot.core.data_type.order_book_tracker_data_source.OrderBookTrackerDataSource._sleep")
    ...
    async def test_listen_for_subscriptions_successful(self, mock_ws, mock_api):
        ...
        msg_queue_funding_info: asyncio.Queue = asyncio.Queue()
        ...
        self.mocking_assistant.add_websocket_aiohttp_message(
            mock_ws.return_value, json.dumps(self.get_ws_funding_info_msg())
        )
        ...
        self.listening_task_funding_info = self.local_event_loop.create_task(
            self.data_source.listen_for_funding_info(msg_queue_funding_info)
        )
        ...
        self.assertEqual(3, len(result.content["bids"]))
        ...
        result_funding_info: FundingInfoUpdate = await msg_queue_funding_info.get()
        self.assertIsInstance(result_funding_info, FundingInfoUpdate)
        self.assertEqual(self.trading_pair, result_funding_info.trading_pair)
        self.assertEqual(Decimal("0.00001793"), result_funding_info.rate)
        self.assertIsInstance(result_funding_info.rate, Decimal)
    ...
    async def test_parse_funding_info_message(self):
        ...
        self.assertEqual(self.trading_pair, funding_info.trading_pair)
        self.assertEqual(Decimal("36717.0"), funding_info.index_price)
        self.assertEqual(Decimal("36733.0"), funding_info.mark_price)
        self.assertEqual(Decimal("0.00001793"), funding_info.rate)

    async def test_parse_funding_info_message_wrong_pair(self):
        """Lines 247: Test _parse_funding_info_message with wrong trading pair"""
        await self._simulate_trading_rules_initialized()
    ...
    async def test_subscribe_to_trading_pair_websocket_not_connected(self):
        ...
        self.assertTrue(
            self._is_logged("WARNING", f"Cannot subscribe to {new_pair}: WebSocket connection not established.")
        )
