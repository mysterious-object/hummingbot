...
import chimerabot.connector.derivative.okx_perpetual.okx_perpetual_web_utils as web_utils
...
from chimerabot.connector.derivative.okx_perpetual import okx_perpetual_constants as CONSTANTS
...
from chimerabot.connector.derivative.okx_perpetual.okx_perpetual_derivative import OkxPerpetualDerivative
...
class OKXPerpetualAPIOrderBookDataSourceTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_subscribe_to_trading_pair_successful(self):
        ...
        self.connector._set_trading_pair_symbol_map(
            bidict({f"{self.base_asset}{self.quote_asset}": self.trading_pair, ex_new_pair: new_pair})
        )
