...
from bin.chimerabot import UIStartListener, detect_available_port
from chimerabot import init_logging
from chimerabot.client.command.start_command import GATEWAY_READY_TIMEOUT
from chimerabot.client.config.config_crypt import BaseSecretsManager, ETHKeyFileSecretManger
...
def autofix_permissions(user_group_spec: str):
    ...
    gateway_path: str = Path.home().joinpath(".chimerabot-gateway").as_posix()
    subprocess.run(
        f"cd '{project_home}' && "
...
async def quick_start(args: argparse.Namespace, secrets_manager: BaseSecretsManager):
    """Start ChimeraBot using unified ChimeraBotApplication in either UI or headless mode."""
    ...
    hb = ChimeraBotApplication.main_application(client_config_map=client_config_map, headless_mode=args.headless)
...
async def load_and_start_strategy(hb: ChimeraBotApplication, args: argparse.Namespace,):
...
async def run_application(hb: ChimeraBotApplication, args: argparse.Namespace, client_config_map):
    """Run the application in headless or UI mode."""
    if args.headless:
        # Re-initialize logging with proper strategy file name for headless mode
        from chimerabot import init_logging
        log_file_name = hb.strategy_file_name.split(".")[0] if hb.strategy_file_name else "chimerabot"
        init_logging("chimerabot_logs.yml", hb.client_config_map,
