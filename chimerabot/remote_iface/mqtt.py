...
from chimerabot import get_logging_conf
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from chimerabot.core.event.event_listener import EventListener  # noqa: F401
...
from chimerabot.core.event import events
from chimerabot.core.event.event_forwarder import SourceInfoEventForwarder
from chimerabot.core.pubsub import PubSub
from chimerabot.core.utils.async_utils import call_sync, safe_ensure_future
from chimerabot.notifier.notifier_base import NotifierBase
from chimerabot.remote_iface.messages import (
    MQTT_STATUS_CODE,
...
mqtts_logger: ChimeraBotLogger = None
...
class MQTTCommands:
    ...
    def _on_cmd_start(self, msg: StartCommandMessage.Request):
        ...
        try:
            ...
            response.msg = f'ChimeraBot start command timed out after {timeout} seconds'
    ...
    def _on_cmd_import(self, msg: ImportCommandMessage.Request):
        ...
        try:
            ...
            response.msg = f'ChimeraBot import command timed out after {timeout} seconds'
            response.status = MQTT_STATUS_CODE.ERROR
        except Exception as e:
            response.status = MQTT_STATUS_CODE.ERROR
            response.msg = str(e)
    ...
    def _on_cmd_status(self, msg: StatusCommandMessage.Request):
        ...
        try:
            ...
            response.msg = f'ChimeraBot status command timed out after {timeout} seconds'
            response.status = MQTT_STATUS_CODE.ERROR
        except Exception as e:
            response.status = MQTT_STATUS_CODE.ERROR
            response.msg = str(e)
...
class MQTTMarketEventForwarder:
    ...
    def logger(cls) -> ChimeraBotLogger:
        ...
        if mqtts_logger is None:  # pragma: no cover
            mqtts_logger = ChimeraBotLogger(__name__)
        return mqtts_logger
...
class MQTTGateway(Node):
    ...
    def __init__(self,
        ...
        self._hb_app: "ChimeraBotApplication" = hb_app
    ...
    def remove_log_handler(self, logger: ChimeraBotLogger):
        logger.removeHandler(self._logh)

    def add_log_handler(self, logger: ChimeraBotLogger):
