...
from chimerabot import root_path

if TYPE_CHECKING:
    from chimerabot import ClientConfigAdapter

_default_paths: Optional["GatewayPaths"] = None
_chimerabot_pipe: Optional[aioprocessing.AioConnection] = None
...
class GatewayPaths:
    ...
    Local paths represent where ChimeraBot client sees the paths from the perspective of its local environment. If
    ChimeraBot is being run from source, then the local environment is the same as the host environment. However, if
    ChimeraBot is being run as a container, then the local environment is the container's environment.

    Mount paths represent where the gateway container's paths are located on the host environment. If ChimeraBot is
    being run from source, then these should be the same as the local paths. However, if ChimeraBot is being run as a
...
def get_gateway_paths(client_config_map: "ClientConfigAdapter") -> GatewayPaths:
    ...
    For ChimeraBot running from source, the gateway files are to be stored in ~/.chimerabot-gateway/<container name>/

    For ChimeraBot running inside container, the gateway files are to be stored in ~/.chimerabot-gateway/ locally;
