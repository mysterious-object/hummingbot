...
from chimerabot.client.config.client_config_map import (
...
from chimerabot.client.config.config_crypt import BaseSecretsManager, store_password_verification
from chimerabot.client.config.config_data_types import BaseConnectorConfigMap
from chimerabot.client.config.config_helpers import ClientConfigAdapter, save_to_yml
from chimerabot.client.config.security import Security
from chimerabot.client.settings import CLIENT_CONFIG_PATH, CONF_DIR_PATH, STRATEGIES_CONF_DIR_PATH
...
def migrate_connector_confs(secrets_manager: BaseSecretsManager):
    ...
    type_dirs: List[DirEntry] = [
        ...
        scandir(f"{root_path() / 'chimerabot' / 'connector'}")
        if f.is_dir()
    ]
        ...
        for connector_dir in connector_dirs:
            ...
            try:
                ...
                util_module_path: str = (
                    f"chimerabot.connector.{type_dir.name}.{connector_dir.name}.{connector_dir.name}_{suffix}"
                )
