...
from chimerabot.client.config.config_data_types import BaseClientModel, ClientConfigEnum
from chimerabot.client.config.config_methods import using_exchange as using_exchange_pointer
from chimerabot.client.config.config_validators import validate_bool, validate_float
from chimerabot.client.settings import DEFAULT_LOG_FILE_PATH, AllConnectorSettings
from chimerabot.connector.connector_base import ConnectorBase
from chimerabot.connector.connector_metrics_collector import (
    DummyMetricsCollector,
    MetricsCollector,
    TradeVolumeMetricCollector,
)
from chimerabot.connector.exchange.binance.binance_utils import BinanceConfigMap
from chimerabot.connector.exchange.gate_io.gate_io_utils import GateIOConfigMap
from chimerabot.connector.exchange.kraken.kraken_utils import KrakenConfigMap
from chimerabot.connector.exchange.kucoin.kucoin_utils import KuCoinConfigMap
from chimerabot.core.rate_oracle.rate_oracle import RATE_ORACLE_SOURCES, RateOracle
from chimerabot.core.rate_oracle.sources.rate_source_base import RateSourceBase
from chimerabot.core.utils.kill_switch import ActiveKillSwitch, KillSwitch, PassThroughKillSwitch

if TYPE_CHECKING:
    from chimerabot.core.trading_core import TradingCore
...
class OpenRouterConfigMap(BaseClientModel):
    api_key: SecretStr = Field(
        default=SecretStr(""),
        description="OpenRouter API key used to access model listings and completions.",
        json_schema_extra={
            "prompt": lambda cm: "OpenRouter API key",
            "is_secure": True,
            "is_connect_key": True,
            "prompt_on_new": True,
        }
    ...
    base_url: str = Field(
        ...
        json_schema_extra={"prompt": lambda cm: "OpenRouter base URL (default https://openrouter.ai/api/v1)"},
    )
    http_referer: str = Field(
        default="",
        description="Optional HTTP-Referer header to include with OpenRouter requests.",
        json_schema_extra={"prompt": lambda cm: "OpenRouter HTTP-Referer header (optional)"},
    )
    app_title: str = Field(
        default="",
        description="Optional X-Title header to include with OpenRouter requests.",
        json_schema_extra={"prompt": lambda cm: "OpenRouter X-Title header (optional)"},
    )
    request_timeout: float = Field(
        default=30.0,
        ge=1.0,
        description="Timeout in seconds for OpenRouter API requests.",
        json_schema_extra={"prompt": lambda cm: "OpenRouter request timeout in seconds (default 30)"},
    )
    models_refresh_interval: int = Field(
        default=3600,
        ge=60,
        description="Seconds to cache model listings before refreshing.",
        json_schema_extra={"prompt": lambda cm: "OpenRouter models refresh interval in seconds (default 3600)"},
    )
    model_config = ConfigDict(title="openrouter")
...
class CoinGeckoRateSourceMode(RateSourceModeBase):
    ...
    api_key: str = Field(
        ...
        json_schema_extra={
            "prompt": lambda cm: "CoinGecko API key (optional, leave empty to use public API) NOTE: will be stored in plain text due to a bug in the way chimerabot loads the config file",
            "prompt_on_new": True,
            "is_connect_key": True,
        },
    )
    ...
    def validate_api_tier(cls, v: str):
        from chimerabot.data_feed.coin_gecko_data_feed.coin_gecko_constants import CoinGeckoAPITier
...
class ClientConfigMap(BaseClientModel):
    instance_id: str = Field(
        ...
        json_schema_extra={"prompt": lambda cm: "Enter a unique identifier for this instance of ChimeraBot"},
    )
    ...
    logger_override_whitelist: List = Field(
        default=["chimerabot.strategy.arbitrage", "chimerabot.strategy.cross_exchange_market_making", "conf"]
    )
    log_file_path: Path = Field(
        default=DEFAULT_LOG_FILE_PATH,
        json_schema_extra={"prompt": lambda cm: "Where would you like to save your logs? (default 'logs/chimerabot_logs.log')"},
    )
    ...
    send_error_logs: bool = Field(
        ...
        json_schema_extra={"prompt": lambda cm: "Would you like to send error logs to chimerabot? (True/False)"},
    )
    ...
    openrouter: OpenRouterConfigMap = Field(default=OpenRouterConfigMap())
