...
from chimerabot.client.config.config_data_types import BaseClientModel, ClientConfigEnum, ClientFieldData
from chimerabot.client.config.config_var import ConfigVar
from chimerabot.client.config.fee_overrides_config_map import fee_overrides_config_map, init_fee_overrides_config
from chimerabot.client.config.gateway_ssl_config_map import SSLConfigMap
from chimerabot.client.settings import (
    CLIENT_CONFIG_PATH,
    CONF_DIR_PATH,
    CONF_POSTFIX,
    CONF_PREFIX,
...
class ClientConfigAdapter:
    ...
    def decrypt_all_secure_data(self):
        from chimerabot.client.config.security import Security  # avoids circular import
...
async def copy_strategy_template(strategy: str) -> str:
    """
    Look up template `.yml` file for a particular strategy in `chimerabot/templates` and copy it to the `conf` folder.
...
def get_strategy_config_map(
    ...
    try:
        ...
        if config_cls is None:  # legacy
            ...
            strategy_module = __import__(f"chimerabot.strategy.{strategy}.{cm_key}",
                                         fromlist=[f"chimerabot.strategy.{strategy}"])
...
def get_strategy_starter_file(strategy: str) -> Callable:
    ...
    `chimerabot/strategy/{STRATEGY_NAME}/start.py` file.
    """
    if strategy is None:
        return lambda: None
    try:
        strategy_module = __import__(f"chimerabot.strategy.{strategy}.start",
                                     fromlist=[f"chimerabot.strategy.{strategy}"])
...
def get_strategy_pydantic_config_cls(strategy_name: str):
    ...
    try:
        ...
        pydantic_cm_path = root_path() / "chimerabot" / "strategy" / strategy_name / f"{pydantic_cm_pkg}.py"
        if pydantic_cm_path.exists():
            pydantic_cm_class_name = f"{''.join([s.capitalize() for s in strategy_name.split('_')])}ConfigMap"
            pydantic_cm_mod = __import__(f"chimerabot.strategy.{strategy_name}.{pydantic_cm_pkg}",
...
async def create_yml_files_legacy():
    """
    Copy `chimerabot_logs.yml` and `conf_global.yml` templates to the `conf` directory on start up
    """
    for fname in listdir(TEMPLATE_PATH):
        if "_TEMPLATE" in fname and CONF_POSTFIX not in fname:
            ...
            if conf_path.endswith("chimerabot_logs.yml"):
