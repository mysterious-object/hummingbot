...
from chimerabot import init_logging
...
from chimerabot.client.ui.interface_utils import start_process_monitor, start_timer, start_trade_monitor
from chimerabot.client.ui.layout import (
    create_input_field,
    create_live_field,
    create_log_field,
    create_log_toggle,
...
from chimerabot.client.ui.stdout_redirection import patch_stdout
...
class ChimeraBotCLI(PubSub):
    ...
    def did_start_ui(self):
        ...
        init_logging("chimerabot_logs.yml", self.client_config_map, override_log_level=log_level)

        self.trigger_event(ChimeraBotUIEvent.Start, self)
    ...
    def handle_tab_command(self, chimerabot: "ChimeraBotApplication", command_name: str, kwargs: Dict[str, Any]):
        ...
        self.display_tab_output(cmd_tab, chimerabot, kwargs)

    def display_tab_output(self,
                           command_tab: CommandTab,
                           chimerabot: "ChimeraBotApplication",
                           kwargs: Dict[Any, Any]):
        if command_tab.task is not None and not command_tab.task.done():
            return
        if threading.current_thread() != threading.main_thread():
            chimerabot.ev_loop.call_soon_threadsafe(self.display_tab_output, command_tab, chimerabot, kwargs)
            return
        command_tab.task = safe_ensure_future(command_tab.tab_class.display(command_tab.output_field, chimerabot,
                                                                            **kwargs))
