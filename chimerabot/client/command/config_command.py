...
from chimerabot.client.command.gateway_command import GatewayCommand
...
from chimerabot.client.config.config_validators import validate_bool, validate_decimal
from chimerabot.client.config.config_var import ConfigVar
from chimerabot.client.config.security import Security
from chimerabot.client.config.strategy_config_data_types import BaseTradingStrategyConfigMap
from chimerabot.client.settings import CLIENT_CONFIG_PATH, STRATEGIES_CONF_DIR_PATH
...
from chimerabot.model.inventory_cost import InventoryCost
from chimerabot.strategy.perpetual_market_making import PerpetualMarketMakingStrategy
from chimerabot.strategy.pure_market_making import PureMarketMakingStrategy
from chimerabot.user.user_balances import UserBalances
...
class ConfigCommand:
    def config(self,  # type: ChimeraBotApplication
               key: str = None,
               value: str = None):
    ...
    def list_configs(self,  # type: ChimeraBotApplication
                     ):
    ...
    def configurable_keys(self,  # type: ChimeraBotApplication
                          ) -> List[str]:
        """
    ...
    async def check_password(self,  # type: ChimeraBotApplication
                             ):
    ...
    async def _config_single_key(self,  # type: ChimeraBotApplication
                                 key: str,
                                 input_value):
        """
    ...
    async def _prompt_missing_configs(self,  # type: ChimeraBotApplication
                                      config_map):
        missings = missing_required_configs_legacy(config_map)
        for config in missings:
