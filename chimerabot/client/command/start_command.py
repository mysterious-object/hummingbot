...
import chimerabot.client.settings as settings
from chimerabot import init_logging
from chimerabot.client.command.gateway_api_manager import GatewayChainApiManager
from chimerabot.client.config.config_validators import validate_bool
...
class StartCommand(GatewayChainApiManager):
    ...
    async def wait_till_ready(self,  # type: ChimeraBotApplication
    ...
    def start(self,  # type: ChimeraBotApplication
    ...
    async def start_check(self,  # type: ChimeraBotApplication
                          log_level: Optional[str] = None,
                          script: Optional[str] = None,
                          conf: Optional[str] = None,
                          is_quickstart: Optional[bool] = False):
        ...
        if script:
            ...
            self._in_start_check = False
            return
        init_logging("chimerabot_logs.yml",
    ...
    async def confirm_oracle_conversion_rate(self,  # type: ChimeraBotApplication
                                             ) -> bool:
        try:
            result = False
