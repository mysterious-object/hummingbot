...
class TabBase(metaclass=ABCMeta):
    ...
    def get_command_arguments(cls) -> Dict[str, Dict[str, Any]]:
        """
        Returns a dictionary of command argument and all its properties. See chimerabot.client.ui.parser for examples.
        """
    ...
    @classmethod
    @abstractmethod
    async def display(cls, output_field: CustomTextArea, chimerabot: "ChimeraBotApplication", **kwargs):
        """
        Displays message on the tab
        :param output_field: The output pane for the tab messages
        :param chimerabot: The current running ChimeraBot application including strategy, connectors and all
