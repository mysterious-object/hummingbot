...
class OrderBookTab(TabBase):
    ...
    async def display(cls,
                      ...
                      market: str = None,
                      live: bool = False):
        if len(chimerabot.markets.keys()) == 0:
            output_field.log("There is currently no active market.")
            return
        if exchange is not None:
            if exchange not in chimerabot.markets:
                output_field.log("Invalid exchange")
                return
            market_connector = chimerabot.markets[exchange]
        else:
            market_connector = list(chimerabot.markets.values())[0]
