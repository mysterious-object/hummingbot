...
from chimerabot import get_strategy_list, root_path
from chimerabot.connector.gateway.common_types import ConnectorType as GatewayConnectorType, get_connector_type
from chimerabot.core.data_type.trade_fee import TradeFeeSchema

if TYPE_CHECKING:
    from chimerabot.client.config.client_config_map import GatewayConfigMap
    from chimerabot.client.config.config_data_types import BaseConnectorConfigMap
...
TEMPLATE_PATH = root_path() / "chimerabot" / "templates"
...
class ConnectorType(Enum):
    """
    The types of exchanges that chimerabot client can communicate with.
    """
...
# GatewayConnectionSetting has been removed - gateway connectors are now configured in Gateway, not ChimeraBot
...
class ConnectorSetting(NamedTuple):
    ...
    def module_path(self) -> str:
        # return connector full path name, e.g. chimerabot.connector.exchange.binance.binance_exchange
        if self.uses_gateway_generic_connector():
            return f"chimerabot.connector.{self.module_name()}"
        return f"chimerabot.connector.{self._get_module_package()}.{self.base_name()}.{self.module_name()}"
...
class AllConnectorSettings:
    ...
    def create_connector_settings(cls):
        ...
        type_dirs: List[DirEntry] = [
            cast(DirEntry, f) for f in scandir(f"{root_path() / 'chimerabot' / 'connector'}")
        ...
        for type_dir in type_dirs:
            ...
            for connector_dir in connector_dirs:
                ...
                try:
                    util_module_path: str = f"chimerabot.connector.{type_dir.name}." \
        ...
        # Gateway connectors are now configured in Gateway, not in ChimeraBot
