...
class ChimeraBotLogger(PythonLogger):
    ...
    def notify(self, msg: str):
        from . import INFO
        self.log(INFO, msg)
        if not ChimeraBotLogger.is_testing_mode():
            from chimerabot.client.chimerabot_application import ChimeraBotApplication
            chimerabot_app: ChimeraBotApplication = ChimeraBotApplication.main_application()
            chimerabot_app.notify(f"({pd.Timestamp.fromtimestamp(int(time.time()))}) {msg}")

    def network(self, log_msg: str, app_warning_msg: Optional[str] = None, *args, **kwargs):
        if app_warning_msg is not None and not ChimeraBotLogger.is_testing_mode():
            from chimerabot.client.chimerabot_application import ChimeraBotApplication
        ...
        if app_warning_msg is not None and not ChimeraBotLogger.is_testing_mode():
            ...
            chimerabot_app: ChimeraBotApplication = ChimeraBotApplication.main_application()
            chimerabot_app.add_application_warning(app_warning)
...
_srcfile = os.path.normcase(ChimeraBotLogger.network.__code__.co_filename)
