...
if TYPE_CHECKING:
    from chimerabot.connector.derivative.bitmart_perpetual.bitmart_perpetual_derivative import (
        BitmartPerpetualDerivative,
    )


class BitmartPerpetualAPIOrderBookDataSource(PerpetualAPIOrderBookDataSource):
    _bpobds_logger: Optional[ChimeraBotLogger] = None
    ...
    async def subscribe_to_trading_pair(self, trading_pair: str) -> bool:
        ...
        try:
            ...
            for channel in stream_id_channel_pairs:
                params = [f"{channel}{f':{symbol.upper()}' if channel != 'futures/ticker' else ''}"]
                payload = {
                    "action": "subscribe",
                    "args": params,
                }
