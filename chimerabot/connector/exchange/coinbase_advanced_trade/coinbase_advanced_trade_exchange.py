...
import chimerabot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_constants as constants
import chimerabot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_web_utils as web_utils
from chimerabot.connector.constants import s_decimal_NaN
from chimerabot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_api_order_book_data_source import (
    CoinbaseAdvancedTradeAPIOrderBookDataSource,
)
from chimerabot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_api_user_stream_data_source import (
    CoinbaseAdvancedTradeAPIUserStreamDataSource,
    CoinbaseAdvancedTradeCumulativeUpdate,
)
from chimerabot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_auth import CoinbaseAdvancedTradeAuth
from chimerabot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_order_book import (
    CoinbaseAdvancedTradeOrderBook,
)
from chimerabot.connector.exchange.coinbase_advanced_trade.coinbase_advanced_trade_web_utils import (
...
from chimerabot.connector.time_synchronizer import TimeSynchronizer
from chimerabot.connector.trading_rule import TradingRule
from chimerabot.connector.utils import TradeFillOrderDetails
from chimerabot.core.data_type.cancellation_result import CancellationResult
...
from chimerabot.core.data_type.trade_fee import AddedToCostTradeFee, TokenAmount, TradeFeeBase
from chimerabot.core.data_type.user_stream_tracker_data_source import UserStreamTrackerDataSource
from chimerabot.core.event.events import MarketEvent, OrderFilledEvent
from chimerabot.core.utils.async_utils import safe_gather
from chimerabot.core.utils.estimate_fee import build_trade_fee
...
class CoinbaseAdvancedTradeExchange(ExchangePyBase):
    ...
    _logger: ChimeraBotLogger | logging.Logger | None = None

    @classmethod
    def logger(cls) -> ChimeraBotLogger | logging.Logger:
        if cls._logger is None:
            name: str = ChimeraBotLogger.logger_name_for_class(cls)
    ...
    async def all_trading_pairs(self) -> List[str]:
        ...
        :return: List of trading pair symbols in the ChimeraBot format
        """
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder) -> bool:
        ...
        if tracked_order.exchange_order_id == "UNKNOWN":
            self.logger().error(f"Failed to cancel order {order_id} without exchange_id: UNKNOWN"
                                "File a bug report with the ChimeraBot team.")
