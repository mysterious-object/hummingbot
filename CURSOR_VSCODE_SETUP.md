## VS Code/Cursor Setup Guide for ChimeraBot Testing

This guide outlines how to configure VS Code or Cursor to efficiently run and debug ChimeraBot tests

**I. Prerequisites:**

* **ChimeraBot Repository:** You have cloned the ChimeraBot repository to your local machine.
* **Conda Environment:** You have created and activated the `chimerabot` Conda environment with all necessary dependencies installed.

**II. Required Files and Configuration:**

Ensure the following files exist in your ChimeraBot project directory with the specified content.

**1. `.env` (Project Root Directory):**

```
PYTHONPATH=${PYTHONPATH}:${PWD}
CONDA_ENV=chimerabot
```

* **`PYTHONPATH`**: This ensures that Python can find the ChimeraBot modules within your project directory.
...
```json
{
    ...
    "python.testing.pytestArgs": [
        ...
        "--ignore=test/chimerabot/connector/derivative/injective_v2_perpetual/",
        "--ignore=test/chimerabot/connector/exchange/injective_v2/",
        ...
        "--ignore=test/chimerabot/client/command/test_create_command.py",
    ],
...
```json
{
    ...
    "configurations": [
        {
            "name": "Python: ChimeraBot",
            "type": "debugpy",
            "request": "launch",
            "program": "${workspaceRoot}/bin/chimerabot.py",
...
* This configuration allows you to run and debug the main ChimeraBot application directly from VS Code/Cursor.

**III. Setup Steps in VS Code/Cursor:**

1.  **Open the ChimeraBot Project:** Open the root directory of your cloned ChimeraBot repository in VS Code or Cursor.

2.  **Select the Python Interpreter:**
    * Open the Command Palette: Press `Ctrl+Shift+P` (Windows/Linux) or `Cmd+Shift+P` (macOS).
    * Type "Python: Select Interpreter" and press Enter.
    * A list of available Python interpreters will appear. **Select the Python interpreter associated with your `chimerabot` Conda environment.** The path should typically include the name of your Conda environment.
...
4.  **Fix Test Discovery (Conda Environment Issue):**
    ...
    * Run the following commands to create a symbolic link to work around a known Conda environment detection issue:
        ```bash
        mkdir -p ~/anaconda3/envs/chimerabot/envs
        ln -s ~/anaconda3/envs/chimerabot/ ~/anaconda3/envs/chimerabot/envs/chimerabot
        ```
        **Note:** Adjust `~/anaconda3/envs/chimerabot` to the actual path of your `chimerabot` Conda environment if it's located elsewhere.
...
1.  **Set Breakpoints:** In your test files or the ChimeraBot code you want to debug, click in the gutter (the space to the left of the line numbers) to set breakpoints.
...
* **`test_create_command.py`:** Tests in `test_create_command.py` are ignored because they modify the `conf_client.yml` file. Running these tests locally can potentially interfere with your ChimeraBot configuration. If you make changes that could affect these commands, ensure your Pull Request (PR) will pass the automated tests, as they might be run in the CI environment.

By following these steps, you can effectively use VS Code or Cursor to run and debug ChimeraBot tests, leveraging the IDE's features for a more integrated and potentially more efficient testing experience, especially when debugging is required. Remember to keep the ignored tests list up-to-date with the `Makefile` to maintain consistency.
